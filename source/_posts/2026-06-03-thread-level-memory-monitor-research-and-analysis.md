...
另一个思路是统计线程的虚拟内存申请释放，重载 malloc/free、new/delete 等接口，使用 TLS 方法记录每个线程的内存情况。使用 PRELOAD 的方式可以做到代码无侵入，但重载增加的记录功能会带来一定的性能损耗，这在低成本硬件上是否可接受需要评估一下。 这里还有一个难点是对于跨线程释放的内存，正确的统计道申请线程上，避免因统计带来的账面内存泄露现象。
